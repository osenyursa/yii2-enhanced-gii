...
foreach ($relations as $name => $rel) {
    ...
    if ($rel[$generator::REL_IS_MULTIPLE] && isset($rel[$generator::REL_TABLE]) && !in_array($name, $generator->skippedRelations)) {
        echo "JsBlock::widget(['viewFile' => '_script', 'pos'=> View::POS_END, \n"
                . "    'viewParams' => [\n"
...
foreach ($relations as $name => $rel) {
    ...
    $class = $generator->nsModel.'\\'.$rel[$generator::REL_CLASS];
    $class = new $class();
    $label = $class::tableLabel();
    if ($rel[$generator::REL_IS_MULTIPLE] && isset($rel[$generator::REL_TABLE]) && !in_array($name, $generator->skippedRelations)) {
        $forms .= "        [\n".
            "            'label' => '<i class=\"glyphicon glyphicon-book\"></i> ' . Html::encode(".$generator->generateString($label)."),\n".
